...
###
### Measure associations between unsupervised clusters of rasters and individual workflow attributes.
###
### source('C:/Kaji/Research Group/ENMs - Plurality of Modeling Workflows (Anna Thonis)/enm_methods_plurality/08_unsupervised_clustering_and_associations_with_workflow_attributes.r')
###
### CONTENTS ###
### setup ###
...
say('############################################################################')
say('### calculate correlation between workflow quality and self-reported AUC ###')
say('############################################################################')
	...
	say('SDM STANDARD RANKS ARE AS 4 ==> gold, 1 ==> deficient... SO POSITIVE CORRELATIONS VALUES ==> WORSE MEETING OF STANDARDS', level = 2)
	odmap_means <- rowMeans(odmap_scores)
	# odmap_means <- 4 - odmap_means # 

	team_codes <- odmap$means$team_code[odmap$means$species == species_full]
	names(odmap_means) <- team_codes

	rast_fields$odmap_mean <- odmap_means[match(rast_fields$team_code, team_codes)]
	rast_fields$eval_metric_auc_roc_value <- as.numeric(rast_fields$eval_metric_auc_roc_value)

	auc_odmap <- rast_fields[ , .(
		eval_metric_auc_roc_value = mean(eval_metric_auc_roc_value, na.rm = TRUE),
		odmap_mean = mean(odmap_mean, na.rm = TRUE)
	), by = team_code]

	auc_odmap <- auc_odmap[complete.cases(auc_odmap)]
	sink(paste0(out_dir, '/Correlation between AUC and Workflow Quality from ODMAP.txt'), split = TRUE)

		say('CORRELATION BETWEEN AUC AND WORKFLOW QUALITY FROM ODMAP FOR ', species_full)
		say('Workflow quality ranked so that higher values ==> better quality. Positive correlation indicates higher AUCs correspond to higher qualities.')
		say(date(), post = 2)
		say('Correlation: ', cor(auc_odmap$eval_metric_auc_roc_value, auc_odmap$odmap_mean))
		say('n = ', nrow(auc_odmap))
